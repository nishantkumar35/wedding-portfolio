...
export function AdminSidebar({ activeTab, setActiveTab, isOpen, setIsOpen }: AdminSidebarProps) {
  return (
    <>
      ...
      <aside className={cn(
        "fixed inset-y-0 left-0 z-50 w-64 flex-shrink-0 flex flex-col bg-[var(--admin-sidebar-bg)] border-r border-[var(--admin-sidebar-border)] transition-transform duration-300 md:relative md:translate-x-0",
        isOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"
      )}>
        {/* Brand */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--admin-sidebar-border)]">
